...
{
    public partial class addBrandYM : SupplerPageBase
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (ChkBrand(model.title, model.companyid, model.id))
            {
            ...
            if (aid > 0)
            {
                ...
                //提交数据后关闭子窗口，父窗口品牌数据重载入
                Response.Write("<script src=\"" + TREC.ECommerce.ECommon.WebSupplerResourceUrl.TrimEnd('/') + "/script/jquery-1.7.1.min.js\" type=\"text/javascript\"></script>");
                Response.Write("<script>if($(\".ymPrompt_close\",parent.document).length>0){"
                    + "$(\"#brandClkLoad\",parent.document).click();"
                    + "$(\".ymPrompt_close\",parent.document).click();}"
                    + "alert('保存品牌信息成功！');</script>");
                Response.End();
            }
