...
namespace TREC.Web.Suppler.brand
{
    public partial class createbrand : SupplerPageBase
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                UiCommon.JscriptPrint(this.Page, "工厂信息获取失败!", Request.Url.AbsoluteUri, "error");
                return;
            }
            ...
            if (brandId > 0)
            {
                ...
                if (aid > 0)
                {
                    ...
                    ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "click", "alert('保存成功！');location.href='brandlist.aspx';", true);
                   // UiCommon.JscriptPrint(this.Page, "编辑成功!", Request.Url.AbsoluteUri, "Success");
                }
                else
                {
                    ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "click", "alert('品牌申请己创建，但代理失败，请查找并申请代理！');location.href='brandlist.aspx';", true);
                    //UiCommon.JscriptPrint(this.Page, "品牌申请己创建，但代理失败，请查找并申请代理!", Request.Url.AbsoluteUri, "error");
                }
            }
            else
            {
                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "click", "alert('品牌添加失败！');location.href='brandlist.aspx';", true);
      //          UiCommon.JscriptPrint(this.Page, "品牌添加失败!", Request.Url.AbsoluteUri, "error");
                return;
            }
