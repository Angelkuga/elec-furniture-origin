<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="AreaSelect.ascx.cs" Inherits="TREC.Web.Suppler.ascx.AreaSelect" %>
<input type="text" value=" " runat="server" id="SelectedValue" name="SelectedValue" style="display:none" /><input type="hidden" value="<%=selectedAreaName%>" id="SelectedText" name="SelectedText" /><span id="<%=ClientID%>Panel"></span>
    ...
    $(function () {
        var self = {
            ...
            loadOptions: function (e) {
                ...
                self.select.createSelect();
                $.get('<%=Request.Path%>?cache=<%=_isCache.ToString().ToLower()%>&action=GetAreaList&areaId=' + valueArray[0], null, function (result) {
                    if (result != "") {
