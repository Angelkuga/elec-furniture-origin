...
function _upfile2222(i) {
    ...
    var fid = i.form.id;
    //file image path
    var filePath = $(obj1).parent().parent().parent().attr('path');
    ...
    //water
    var mywater = $(obj1).parent().parent().parent().attr('iswater');
    if (mywater == undefined) {
        mywater = '';
    }
    if (mywater == '') {
        mywater = '1';
    ...
    if (mywater != '') {
        filePath += '&iswater=' + mywater.toString();
    }

    //width
    var sWidth = $(obj1).parent().parent().parent().attr('sWidth');
    if (sWidth == undefined) {
        sWidth = '';
    }
    if (sWidth != '') {
        filePath += '&sWidth=' + sWidth.toString();
    }

    //height
    var sHeight = $(obj1).parent().parent().parent().attr('sHeight');
    if (sHeight == undefined) {
        sHeight = '';
    }
    if (sHeight != '') {
        filePath += '&sHeight=' + sHeight.toString();
    }
    var strUrl = _strBasePath() + "ajaxtools/fileupload.ashx?f=" + $(obj1).attr("name") + "&t=add&iswater=" + iswater;
    //var strUrl = _strBasePath() + "Member/MemberAjax/FileUploadHandler.ashx?f=" + $(obj1).attr("name") + filePath + "&t=add";
    $("#" + fid).ajaxSubmit({
        ...
        success: function (data) {
            ...
            $.each(data, function (ls) {
                ...
                if (data[ls].msg == "0") {
                    ...
                    /*有些地方不要“浏览”跟“删除”项*/
                    var isNoTip = $(obj1).parent().parent().parent().find("input[type='hidden']").hasClass("notip");
                    if (!isNoTip) {
...
function _updel(i, t) {
    ...
    var submitUrl = _strBasePath() + "Member/MemberAjax/FileUploadHandler.ashx?f=" + $(obj1).parent().find("input.filedisplay").attr("value") + "&t=del&dt=" + t;
    $.ajax({
        url: submitUrl,
        timeout: 600000,
        dataType: "json",
...
$(function () {
    ...
    $.each($(".fileUpload"), function () {
        ...
        if ($(this).find("input[type='hidden']").attr("value") != "" && $(this).find("input[type='hidden']").attr("value").length > 0) {
            //有些地方不要提示删除跟浏览
            var isNoTip = $(this).find("input[type='hidden']").hasClass("notip");
            if (!isNoTip) {
                ...
                if (!$(this).hasClass("m")) {
                    ...
                    getHtmlTools($(this).find("input[type='file']"), "edit_" + $(this).attr("id"));
                } else {
                    for (var n = 0; n < arrFileList.length; n++) {
                        ...
                        if (arrFileList[n] != "") {
                            ...
                            getHtmlTools($(this).find("input[name='" + "nf" + "_" + j + "_" + n + "']"), "edit_" + $(this).attr("id"));
                        }
                    }
                }
            }
