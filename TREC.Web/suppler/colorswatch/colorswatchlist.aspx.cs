...
{
    public partial class colorswatchlist : SupplerPageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (memberInfo != null)
                {
                    ...
                    string myurl = "colorswatchlist.aspx?";
                    //色板名称
                    if (!string.IsNullOrEmpty(Request["SwatchName"]))
                    {
                        ...
                        strWhere += " and SwatchName like '%" + SwatchName + "%'" ;
                    }
                    ...
                    if (!string.IsNullOrEmpty(Request["seriesId"]))
                    {
                        ...
                        if (0 < seriesId)
                        {
                            ...
                            strWhere += " and bSeriesId=" + seriesId;
                        }
                        else
                            seriesId = 0;
                    }
                    ...
                    //总页数
                    int PageCount = 0;
                    List<t_colorswatch> colorswatchList = ECBrand.GetColorSwtchList(ECommon.QueryPageIndex, pagequantity, strWhere, out tmpPageCount);
                    if (0 < tmpPageCount)
                    {
                        RptSwatch.DataSource = colorswatchList;
                        RptSwatch.DataBind();
                        int totalPage = 0;
                        //计算总页数
                        ...
                        pageStr = commonMemberPageSub(tmpPageCount, totalPage, pagequantity, currentPage, "brandlist.aspx?", 5, "个", "色板");
                    }
