...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="maincon">
        <div class="maintabcor">
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tr>
                    <td height="35" width="60" align="right">
                        组合编号
                    </td>
        ...
        <option value="">请选择</option></select>--%>
                ...
                <tr>
                    ...
                    <td colspan="8"  style="text-align:left;">
                        <input type="text" id="productSKN" style=" width:170px;" />
                        <input type="button" class="btnadd"  id="addProductPOP" value="搜索" />
                        <input type="hidden" id="loadProd" />
                    </td>
                </tr>
            </table>
        </div>
    ...
    <input id="loadProd" type="text"  style="width:500px;display:none;" />
    <div class="maintabcor bordtop bordbottom">
        <table width="100%" border="0" cellpadding="0" cellspacing="0">
            <tr>
                ...
                <td width="12%" style="text-align: right;">参加活动</td>
                <td style="text-align: left;"><asp:DropDownList ID="ddlGroupProductPromotion" runat="server" CssClass="select">
                        </asp:DropDownList></td>
            </tr>
        </table>
    </div>
    ...
    <!--
        $(function () {
            ...
            //添加单品弹出iframe
            $("#addProductPOP").click(function addProductYmprompt() {
                var gpId = '<%=gpId %>';
                if (gpId == 0) {
                    gpId = "";
                }
                ymPrompt.win({
                    message: "/suppler/product/addProductPOP.aspx?productSKN=" + $("#productSKN").val() +
                    "&brand="+ $('#<%=ddlbrand.ClientID %> option:selected').val() + 
                    "&BigCategory="+ $('#<%=ddlBigCategory.ClientID %> option:selected').val() +
                    "&style=" + $('#<%=ddlstyle.ClientID %> option:selected').val() +
                    "&material=" + $('#<%=ddlmaterial.ClientID %> option:selected').val() +
                    "&SeriesId=" + $('#<%=SeriesId.ClientID %> option:selected').val() +
                    "&smallCateId=" + $('#<%=smallCateId.ClientID %> option:selected').val() +
                    "&IsGroup=" + $('input[name=IsGroup]:checked').val()+
                    "&gpId=" + gpId
                , width: 700, height: 600, title: '套组合添加单品', handler: function () { }, iframe: true });
            });

            //装在产品数据
            $("#loadProd").click(function () {
                $.get('../ajax/MemberHandler.ashx',
                        { Action: 'searchproductbyid',
                            IdList: $("#<%=hiddSingProductID.ClientID %>").val()
                        },
                         function (data) {
                             $("#selectProdect tbody").html(data.split('$')[0]);
                             $("#selectProdect").show();
                         });
            });
            //产品输入框点击事件
            ...
            $.get('../ajax/MemberHandler.ashx', { Action: 'copygroupproduct', productNo: escape(groupProductNo.toString()), ran: Math.random() }, function (data) {
                ...
                if (data != null) {
                    if (data == 'no') {
                        ...
                        var myhtml = '<table width="100%" border="0" cellspacing="0" cellpadding="0"><tr><td>选择</td><td>名称</td><td>编号</td><td>商铺</td></tr>';
                        myhtml += data;
                        myhtml += '</table>';
