...
{
    public class promotions : CompanyPageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect(string.Format(EnUrls.CompanyInfoPromotionsSearch, ECommon.QueryCId, companyInfo.CompanyBrandInfo.id, "0", 1));
            }
            pageTitle = "-" + companyInfo.title + "-促销信息";
            if (!IsPostBack)
            {
                showData();
            }
        ...
        void showData()
        {
            var query = dpromotions.Linq.Where(c => c.auditstatus == 1 && c.attribute == "促销" && c.linestatus == 1 && (c.membertype == (int)EnumMemberType.工厂企业 || c.membertype == (int)EnumMemberType.经销商)).GroupJoin(dpromotions.LinqData<Mpromotionsrelated>(), c => c.id, c => c.promotionsid, (f, k) => new { f, k });
