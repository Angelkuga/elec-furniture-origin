...
function _upfile(i) { 
    ...
    var obj1 = $(i);
    var fid = i.form.id;
    var strUrl =  "/ajaxtools/AliyunImgfileupload.ashx?f=" + $(obj1).attr("name") + "&t=add&iswater=" + iswater;
            ...
            $.each(data, function (ls) {
                ...
                if (data[ls].msg == "0") {
                    ...
                    if ($(obj1).parent().parent().find("input.filedisplay").attr("value") != "") {
                        if ($(obj1).parent().parent().parent().find("input[type='hidden']").val().indexOf(',') < 0) {
                            $(obj1).parent().parent().parent().find("input[type='hidden']").val(data.msbox);
                        }
                        else {//
                            var newimg = data.msbox; //新上传的图片
                            var oldimg = $(obj1).parent().parent().find("input.filedisplay").attr("value");//原图片
                            var imgvalue = $(obj1).parent().parent().parent().find("input[type='hidden']").val();//存图片的位置
                            $(obj1).parent().parent().parent().find("input[type='hidden']").val(imgvalue.replace(oldimg,newimg));
                           
                        }
                    } else {
                        if ($(obj1).parent().parent().parent().find("input[type='hidden']").val() == "")//不为空时的添加 
                        {
                            if ($(obj1).parent().parent().parent().find("input[type='hidden']").val().indexOf(',') < 0) {
                                $(obj1).parent().parent().parent().find("input[type='hidden']").val(data.msbox);
                            }
                            ...
                            if ($(obj1).parent().parent().parent().find("input[type='hidden']").val().indexOf(',') < 0) {
                                $(obj1).parent().parent().parent().find("input[type='hidden']").val($(obj1).parent().parent().parent().find("input[type='hidden']").val() + ',')
                            }
...
function _updel(i, t) {
    var obj1 = $(i);
   // $(obj1).parent().parent().find("input[type='hidden']").val(data.path);
    if ($(obj1).parent().parent().hasClass("m")) {
        var oldimg = $(obj1).parent().find("input.filedisplay").attr("value"); //原图片
        var imgvalue = $(obj1).parent().parent().find("input[type='hidden']").val(); //存图片的位置
        $(obj1).parent().parent().find("input[type='hidden']").val(imgvalue.replace(oldimg + ',', '').replace(oldimg, ''));
        $(obj1).parent().remove();
    }
    else {
        $(obj1).parent().parent().find("input[type='hidden']").val("");
...
function _updel222(i, t) {    
    var obj1 = $(i);
    //$(obj1).parent().find("input[type='file']").attr("disabled", "true");
    var submitUrl = _strBasePath() + "ajaxtools/AliyunImgfileupload.ashx?f=" + $(obj1).parent().find("input.filedisplay").attr("value") + "&t=del&dt=" + t;
    //alert(submitUrl);
    $.ajax({
        url: submitUrl,
        timeout: 600000,
...
function _upsee(i, t) {
    if ($(i).text() == "预览") {
        ...
        if (t.indexOf("edit_") < 0) {
            objImg =  $(obj1).parent().find("input.filedisplay").attr("value");
        }
        else {
            objImg =  $(obj1).parent().find("input.filedisplay").attr("value");
        }
...
function getHtmlTools(o, t) {
    var del = "<a href=\"javascript:;\" class=\"_filedel\" onclick=\"_updel(this,'" + t + "')\">删除</a>";
   
    $(o).parent().parent().find("a._filedel,a._filesee").remove();
    //if ($(o).parent().parent().parent().hasClass("m")) {
        $(o).parent().parent().append(del+"<a href=\"javascript:;\" class=\"_filesee\"  onclick=\"_upsee(this,'" + t + "')\">预览</a>");
   // }
   // else {
      //  $(o).parent().parent().append("<a href=\"javascript:;\" class=\"_filesee\"  onclick=\"_upsee(this,'" + t + "')\">预览</a>");
   // }
}
