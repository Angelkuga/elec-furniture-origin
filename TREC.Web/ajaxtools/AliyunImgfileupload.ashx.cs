...
using TREC.ECommerce.Ui;

namespace TREC.Web.ajaxtools
{
    /// <summary>
    /// AliyunImgfileupload1 的摘要说明
    /// </summary>
    public class AliyunImgfileupload1 : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (upFileType == "add")
            {
                ...
                int iswater = 2;
                AliyunECUpLoad ecUpload;
                if (!string.IsNullOrEmpty(_iswater))
                {
                    iswater = Convert.ToInt32(_iswater);
                    ecUpload = new AliyunECUpLoad(iswater);
                }
                else
                {
                    ecUpload = new AliyunECUpLoad();
                }
