...
<body> 
    ...
    <div class="main clearfix">
        ...
        <div class="listy">
            <div class="listy1" style='height: 20px;'>
                目前搜索到<span id='licount'></span> 件相关商品&nbsp;&nbsp;&nbsp;&nbsp;<span id='spans'><span
                    id='spantid'></span><span id='spanfg'></span><span id='spancz'></span><span id='spanys'></span><span
                        id='spanpp'></span><span id='spanmc'></span></span><a href="javascript:selall();">清空查询条件</a></div> <%-- <a href="#" onclick='selall();'>清空查询条件</a>--%>
            <div class="listy2">
                <%=lx %>
                <%--<dl>
                ...
                <dl>
                    ...
                    <dd id="ddpp">
                        <div class="pplm">
                            <ul class="tabs" style='width: 840px' id="tabs2">
    ...
    <script>
        ...
        $.each($("#con_one_1 a"), function (i, link) {
            ...
            if ($(this).html().replace(/ /g, "").indexOf("卧室套组合") != -1) {
                link.href = "productlist2.aspx?did=7&ty=" + escape("卧室套组合"); // link.href = "product/list2---------1--7-64.aspx?ty=" + escape("卧室套组合");
            }
            if ($(this).html().replace(/ /g, "").indexOf("客厅套组合") != -1) {
                link.href = "productlist2.aspx?did=9&ty=" + escape("客厅套组合"); //link.href = "product/list2---------1--9-65.aspx?ty=" + escape("客厅套组合");
            }
            if ($(this).html().replace(/ /g, "").indexOf("餐厅套组合") != -1) {
                link.href = "productlist2.aspx?did=10&ty=" + escape("餐厅套组合"); //link.href = "productlist2---------1--10-66.aspx?ty=" + escape("餐厅套组合");
            }
            if ($(this).html().replace(/ /g, "").indexOf("书房套组合") != -1) {
                link.href = "productlist2.aspx?did=11&ty=" + escape("书房套组合"); //link.href = "productlist2---------1--11-67.aspx?ty=" + escape("书房套组合");
            }
            if ($(this).html().replace(/ /g, "").indexOf("儿童套组合") != -1) {
                link.href = "productlist2.aspx?did=12&ty=" + escape("儿童套组合"); //link.href = "product/list2---------1--12-68.aspx?ty=" + escape("儿童套组合");
            }
        ...
        function hd(index) {
            try {
                //小类/风格/材质/颜色/类型
                var cv = $("#hidxl").val() + "," + $("#hidfg").val() + "," + $("#hidcz").val() + "," + $("#hidys").val() + ",";
                setCookie("kshis", cv, 1000);
            }
            catch (e) { }
            $.ajax({
                async: false, //是否异步
                url: '../../../ajax/hdsearch.ashx?t=ncp&pg=' + pagesize + '&pi=' + index + '&bid=' + $("#hiddid").val() + '&did=' + $("#hidtid").val() + '&xl=' + $("#hidxl").val() + '&fg=' + $("#hidfg").val() + '&cz=' + $("#hidcz").val() + '&ys=' + $("#hidys").val() + '&pp=' + $("#hidpp").val() + '&mc=' + $("#hidmc").val() + '&k=' + escape($("#hidkey").val()) + '&m=' + Math.random(),
                type: 'post', //post方法
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < data.dt.length; i++) {
                        ...
                        s += "<div class='listp' style='height:285px;'><a target='_blank' href='<%=TREC.ECommerce.ECommon.WebUrl %>/productinfo.aspx?id=" + data.dt[i].tpid + "'>";
                        ...
                        s += "<a  target='_blank' href='<%=TREC.ECommerce.ECommon.WebUrl %>/productinfo.aspx?id=" + data.dt[i].tpid + "' title='" + titlenames + "'>" + titlenamesub + "</a>";
                        s += "<br>";
                        ...
                        if (parseFloat(data.dt[i].saleprice) > 0) {
                            if (parseFloat(data.dt[i].buyprice) > 0) {
                                s += "<br /><span class='huis'>销售价：¥" + data.dt[i].saleprice + "</span>";
                            }
                            else {
                        ...
                        if (parseFloat(data.dt[i].buyprice) > 0) {
                            s += "<br /><span class='pricered'>"+data.dt[i].buypriceName+"：¥" + data.dt[i].buyprice + "</span>";
                        }
                        s += "<br>";
        ...
        $.ajax({
            async: false, //是否异步
            url: '../../../ajax/hdsearch.ashx?t=tjcp&did=' + $("#hiddid").val() + '&xid=' + $("#hidxl").val() + '&ty=0&p=102&c=5&m=' + Math.random(),
            type: 'post', //post方法
            dataType: 'json', //返回json格式数据
            success: function (data) {
                ...
                for (var i = 0; i < data.dt.length; i++) {
                    ...
                    if (parseFloat(data.dt[i].buyprice) > 0) {
                        s += "<br /><span class='pricered'>促销价：¥" + data.dt[i].buyprice + "</span>";
                    }
                    s += "    <br>";
                    s += "</div>";
                }
