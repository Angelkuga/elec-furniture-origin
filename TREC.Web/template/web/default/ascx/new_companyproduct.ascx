                    ...
                    if (($("#pi_brand").val() == "" || $("#pi_brand").val() == null) && ($("#pi_brands").val() == "" || $("#pi_brands").val() == null) && ($("#pi_pcategory").val() == "" || $("#pi_pcategory").val() == null)) {
                               ...
                               if (data != "{}" || data!="") 
                               {
                                   ...
                                   if (data[i].type == "category") 
                                   {
                                       if(bo)
                                       {
                                           ...
                                           {
                                               $("#pi_categorylist").append("<a style=\"color:Red;font-weight:bold;\" href=\"<%=TREC.ECommerce.ECommon.WebUrl %>/company/" + <%=companyInfo.id %> + "/product.aspx\">所有类型</a>");
                                           }
                                           else
                                           {
                                               $("#pi_categorylist").append("<a href=\"<%=TREC.ECommerce.ECommon.WebUrl %>/company/" + <%=companyInfo.id %> + "/product.aspx\">所有类型</a>");
                                           }
                                           bo=false;
                                       }
                                       if("<%=TREC.ECommerce.CompanyPageBase.setvalue %>"==data[i].v)
                                       {
                                           $("#pi_categorylist").append("<a style=\"color:Red;font-weight:bold;\" href=\"<%=TREC.ECommerce.ECommon.WebUrl %>/company/" + <%=companyInfo.id %> + "/product-"+$("#pi_brand").val()+"--------1--" + data[i].pc + "-"+data[i].v+"--"+$("#pi_brands").val()+".aspx\">" + data[i].n + "(" + data[i].count + ")</a>");
                                        }
                                       else
                                       {
                                           $("#pi_categorylist").append("<a href=\"<%=TREC.ECommerce.ECommon.WebUrl %>/company/" + <%=companyInfo.id %> + "/product-"+$("#pi_brand").val()+"--------1--" + data[i].pc + "-"+data[i].v+"--"+$("#pi_brands").val()+".aspx\">" + data[i].n + "(" + data[i].count + ")</a>");
                                       }
                                   }
                                   });
                        ...
                        $.each(data, function (i) {
                            if (data[i].type == "category") 
                            {
                                ...
                                if("<%=TREC.ECommerce.CompanyPageBase.setvalue %>"==data[i].v)
                                {
                                    $("#pi_categorylist").append("<a style=\"color:Red;font-weight:bold;\" href=\"<%=TREC.ECommerce.ECommon.WebUrl %>/company/" + <%=companyInfo.id %> + "/product---------1--" + data[i].pc + "-"+data[i].v+".aspx\">" + data[i].n + "(" + data[i].count + ")</a>");
                                }
                                else
                                {
                                    $("#pi_categorylist").append("<a href=\"<%=TREC.ECommerce.ECommon.WebUrl %>/company/" + <%=companyInfo.id %> + "/product---------1--" + data[i].pc + "-"+data[i].v+".aspx\">" + data[i].n + "(" + data[i].count + ")</a>");
                                }
                            }
                        });
                    }
