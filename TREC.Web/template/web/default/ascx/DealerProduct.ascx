<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="DealerProduct.ascx.cs" Inherits="TREC.Web.template.web.ascx.DealerProduct" %>
                               ...
                               if (data != "{}" || data!="") 
                               {
                                   ...
                                   if (data[i].type == "category") 
                                   {
                                       if(bo)
                                       {
                                           if("<%=setvalue %>"=="-1")
                                           {
                                               $("#pi_categorylist").append("<li><a style=\"color:Red;font-weight:bold;\" href=\"<%=TREC.ECommerce.ECommon.WebUrl %>company/" + <%=getdid %> + "/product.aspx\">所有类型</a></li>");
                                           }
                                           else
                                           {
                                               $("#pi_categorylist").append("<li><a href=\"<%=TREC.ECommerce.ECommon.WebUrl %>Dealer/" + <%=getdid %> + "/product.aspx\">所有类型</a></li>");
                                           }
                                           bo=false;
                                            $("#pi_categorylist").append("<li><a  href=\"<%=TREC.ECommerce.ECommon.WebUrl %>shop/" + <%=getdid%> + "/producttzh.aspx\">套组合</a></li>");
                                       }
                                       if("<%=setvalue %>"==data[i].v)
                                       {
                                           $("#pi_categorylist").append("<li><a style=\"color:Red;font-weight:bold;\" href=\"<%=TREC.ECommerce.ECommon.WebUrl %>Dealer/" + <%=getdid %> + "/product-"+$("#pi_brand").val()+"--------1--" + data[i].pc + "-"+data[i].v+"--"+$("#pi_brands").val()+".aspx\">" + data[i].n + "(" + data[i].count + ")</a></li>");
                                        }
                                       else
                                       {
                                           $("#pi_categorylist").append("<li><a href=\"<%=TREC.ECommerce.ECommon.WebUrl %>Dealer/" + <%=getdid %> + "/product-"+$("#pi_brand").val()+"--------1--" + data[i].pc + "-"+data[i].v+"--"+$("#pi_brands").val()+".aspx\">" + data[i].n + "(" + data[i].count + ")</a></li>");
                                       }
                                   }
                                   });
                        ...
                        $.each(data, function (i) {
                            if (data[i].type == "category") 
                            {
                                if(bo)
                                {
                                    ...
                                    {
                                        $("#pi_categorylist").append("<li><a style=\"color:Red;font-weight:bold;\" href=\"<%=TREC.ECommerce.ECommon.WebUrl %>Dealer/" + <%=getdid %> + "/product.aspx\">所有类型</a></li>");
                                    }
                                    else
                                    {
                                        $("#pi_categorylist").append("<li><a href=\"<%=TREC.ECommerce.ECommon.WebUrl %>Dealer/" + <%=getdid %> + "/product.aspx\">所有类型</a></li>");
                                ...
                                {
                                    $("#pi_categorylist").append("<li><a style=\"color:Red;font-weight:bold;\" href=\"<%=TREC.ECommerce.ECommon.WebUrl %>Dealer/" + <%=getdid %> + "/product---------1--" + data[i].pc + "-"+data[i].v+".aspx\">" + data[i].n + "(" + data[i].count + ")</a></li>");
                                }
                                else
                                {
                                    $("#pi_categorylist").append("<li><a href=\"<%=TREC.ECommerce.ECommon.WebUrl %>Dealer/" + <%=getdid %> + "/product---------1--" + data[i].pc + "-"+data[i].v+".aspx\">" + data[i].n + "(" + data[i].count + ")</a></li>");
                                }
                            }
                        });
                    }
