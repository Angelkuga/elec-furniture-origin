<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="headerCompany.ascx.cs" Inherits="TREC.Web.aspx.ascx.headerCompany" %>
...
<div class="header-box">
    ...
    <div class="w clearfix" style="border-top-color : #D8D8D8;border-top-style : dashed;border-top-width : 1px;padding-top:15px;margin-top:15px;">
            <h4 class="yahei fl"><%=companyInfo.title %></h4>
            <%foreach (CompanyBrand b in companyInfo.CompanyBrandList)
              { %>
            <a class="fr" title="<%=companyInfo.BrandName.Replace(",", " ") %>" href="<%=string.Format(EnUrls.CompanyInfoIndex,ECommon.QueryCId)%>">
...
<div class="nav">
    <div class="nav-box w clearfix">        
        <ul class="nav-title fl" id="j_menuTab">
            <li class="<%=TREC.ECommerce.UiCommon.QueryStringCur("pageName", "index.aspx", "", "on")%>"><a class="nav-item" href="<%=string.Format(EnUrls.CompanyInfoIndex,ECommon.QueryCId) %>">首 页</a></li>
            <li class="<%=TREC.ECommerce.UiCommon.QueryStringCur("pageName", "about.aspx", "", "on")%>"><a class="nav-item" href="<%=string.Format(EnUrls.CompanyInfoAbout,ECommon.QueryCId) %>">
                    厂商介绍</a></li>
            <%
                if (companyInfo.CompanyBrandList != null && companyInfo.CompanyBrandList.Count != 0)
                {
            %>
                <li class="<%=TREC.ECommerce.UiCommon.QueryStringCur("pageName", "brand.aspx", "", "on")%>"><a class="nav-item" href="<%=string.Format(EnUrls.CompanyInfoBrand,ECommon.QueryCId) %>">
                    品牌介绍</a></li>
                <li class="<%=TREC.ECommerce.UiCommon.QueryStringCur("pageName", "product.aspx", "", "on")%>"><a class="nav-item" href="<%=string.Format(EnUrls.CompanyInfoProduct,ECommon.QueryCId) %>">
                    全部产品</a></li>
            <%} %>
            <li class="<%=TREC.ECommerce.UiCommon.QueryStringCur("pageName", "merchants.aspx,merchantsInfo.aspx", "", "on")%>"><a class="nav-item" href="<%=string.Format(EnUrls.CompanyMerchantsList, ECommon.QueryCid) %>">
                            招商信息</a></li>
            <li class="<%=TREC.ECommerce.UiCommon.QueryStringCur("pageName", "promotions.aspx,promotionsinfo.aspx", "", "on") %>"><a class="nav-item" href="<%=string.Format(EnUrls.CompanyInfoPromotions, ECommon.QueryCid) %>">
                            促销信息</a></li>
            <li class="<%=TREC.ECommerce.UiCommon.QueryStringCur("pageName", "news.aspx,newsinfo.aspx", "", "on")%>"><a class="nav-item" href="/news/list.aspx">
                            公司新闻</a></li>
            <li class="<%=TREC.ECommerce.UiCommon.QueryStringCur("pageName", "address.aspx", "", "on")%>"><a class="nav-item" href="<%=string.Format(EnUrls.CompanyInfoAddress,ECommon.QueryCId) %>" target="_blank">
                            店铺地址</a></li>
        </ul>
    </div>
</div>
