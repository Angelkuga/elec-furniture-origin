...
{
    public partial class OrderListInfor : PayBase
	{
        ...
        public string getsend(string proinforid)
        {
            try
            {
                int CustomerUserId = Int32.Parse(TRECommon.CookiesHelper.GetCookieCustomer("cid"));
                OrderInfor _orderinfor = new OrderInfor();
                _orderinfor = EntityOper.OrderInfor.Where(p => p.CustomerUserId == CustomerUserId && p.OrderNumber == getOrderNumber && p.productattributeId.Value == SubmitMeth.getInt(proinforid)).FirstOrDefault();

                if (_orderinfor != null && _orderinfor.Deliverystatus != null)
                {
                    return Deliverystatus.Where(p => p.Key == _orderinfor.Deliverystatus.Value.ToString()).FirstOrDefault().Value;
                }
                else
                {
                    return reget;
