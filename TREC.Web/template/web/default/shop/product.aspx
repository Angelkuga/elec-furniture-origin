<%@ Page Language="C#" AutoEventWireup="true"  Inherits="TREC.Web.aspx.shop.product" %>
      ...
      <div class="homebraLi2" style="margin-top:0;">
        <div class="productList12">
          <div class="productList121">
            <ul class="pr12-ti">
              ...
              <li><a class=""                                                                                                                    href="<%=string.Format(EnUrls.ShopInfoProductListBrandstzh, ECommon.QuerySId, ECommon.QuerySearchBrand, ECommon.QuerySearchStyle, ECommon.QuerySearchMaterial, ECommon.QuerySearchSpread, ECommon.QuerySearchStaffsize, "", ECommon.QuerySearchComposing, ECommon.QuerySearchKey, ECommon.QueryPageIndex, ECommon.QuerySearchType, "999", "",ECommon.QuerySearchPostName,ECommon.QuerySearchBrands) %>" >套组合产品</a></li>
            </ul>
        ...
        <div class="productLi_w">
          <%foreach (EnWebProduct p in list)
            {
                %>
        <div class="productPa2 productLit c61f38" style="overflow:hidden;">
          <p>
           <center>
          ...
          <img alt="<%=p.brandtitle + " " +p.materialname + " " + p.stylename+" "+p.categorytitle+" "+ p.sku %>" src="<%=EnFilePath.GetProductThumbPathNew(p.id.ToString(),p.thumb,"230","173") %>"  style='height:173px;' onload="javascript:DrawImage(this,230,173);"  /></a></center></p>
          <p class="titlepro"><a href="<%=string.Format(EnUrls.ProductInfo,p.id) %>" target="_blank"><%=p.HtmlProductName %></a></p>
          <p>系列：<span><span style="color:#C61F38;"><%=p.brandstitle %></span>&nbsp;</span>风格：<span><span style="color:#C61F38;"><%=p.stylename %></span></span></p>
          <%if (p.ProductAttributeInfo.markprice != null && decimal.Parse(p.ProductAttributeInfo.markprice) > 0)
           {%>
          <h4 style="font-size:12px;color:#bfbfbf;"><strong >市场参考价：￥</strong><a style="text-decoration:none; cursor:default; font-weight:bold;color:#bfbfbf;"><%=p.ProductAttributeInfo.markprice%> 元</a></h4>
             <%} %>
       
          <%if (p.ProductAttributeInfo.saleprice != null && decimal.Parse(p.ProductAttributeInfo.saleprice) > 0)
                                                              {%>
         <h4 style="font-size:12px;color:#b80b0f;"><strong>销售价:￥</strong><a style="text-decoration:none; cursor:default; font-weight:bold;color:#b80b0f;"><%=p.ProductAttributeInfo.saleprice%>元</a></h4>
                                                        <%} %>
                                                        
                                                        <%if (p.ProductAttributeInfo.buyprice != null && decimal.Parse(p.ProductAttributeInfo.buyprice) > 0)
                                                              {%>
          <h4 style="font-size:12px;color:#b80b0f;"><strong><%=ECProduct.CheckbuypriceName(p.id.ToString()) %>:￥</strong><a style="text-decoration:none; cursor:default; font-weight:bold;color:#b80b0f;"><%=p.ProductAttributeInfo.buyprice%>元</a></h4>
                                                        <%} %>
